...
package body Ocarina.Backends.Build_Utils is
   ...
   package body Makefiles is
      ...
      Saved_Makefile : Makefile_Type;
      Use_Saved_Makefile : Boolean := False;
                  ...
                  while Present (D) loop
                     ...
                     then
                        ...
                        while Present (F) loop
                           ...
                           if Present (J) then
                              Use_Saved_Makefile := True;
                              Saved_Makefile := M;
                              Visit (Parent_Component (J));
                              Use_Saved_Makefile := False;
                           end if;
                           F := Next_Node (F);
                        end loop;
                     end if;
         ...
         if not AAU.Is_Empty (Features (E)) then
            ...
            while Present (F) loop
               if Present (Corresponding_Instance (F)) then
                  ...
                  then
                     Simulink_Dir :=
                       Get_Source_Text (Corresponding_Instance (F)) (1);
      ...
      begin
         ...
         then
            ...
            Write_Line (" \");
            Write_Str (ASCII.HT & "   ada-start.o ");
         end if;
